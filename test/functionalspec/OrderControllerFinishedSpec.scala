...
class OrderControllerFinishedSpec extends PlaySpec with Context with BaseData with GuiceOneAppPerSuite {
  ...
  "OrderControllerFinishedSpec" should {
    ...
    "check qty of product decreased" in {
      val response    = Page(
        data  = List(Mapper.map[Product, ProductDto](product.copy(qty = product.qty - 10))),
        page  = 1,
        size  = 10,
        sort  = "asc",
        sortBy= "id" ,
    ...
    "check qty of coupon decreased" in {
      val response    = Page(
        data  = List(Mapper.map[Coupon, CouponDto](coupon.copy(qty = coupon.qty -1))),
        page  = 1,
        size  = 10,
        sort  = "asc",
        sortBy= "id" ,
