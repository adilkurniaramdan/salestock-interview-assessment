...
class ProductRepositoryMemory @Inject()()(implicit ec: ExecutionContext) extends ProductRepository {
  ...
  override def page(page: Int, size: Int, sort: String, sortBy: String, filter: String): Future[(List[Product], Int)] = future{
  ...
  private def generateId() =
    if(storage.isEmpty) 1L else storage.keys.max + 1
